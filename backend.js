...
/*
   ...
   from the Home page — separate from the four
   on-site safety zones above, which track the
   immediate temple/event grounds only.
*/
...
const translations = {
    ...
    en: {
        ...
        advisoryPlaceholder:
            "e.g. Main Temple zone is at full capacity — please use the East entrance.",
    ...
    hi: {
        ...
        advisoryPlaceholder:
            "उदा. मुख्य मंदिर क्षेत्र पूर्ण क्षमता पर है — कृपया पूर्वी प्रवेश द्वार का उपयोग करें।",
    ...
    bn: {
        ...
        advisoryPlaceholder:
            "যেমন: মূল মন্দির অঞ্চল পূর্ণ ধারণক্ষমতায় আছে — অনুগ্রহ করে পূর্ব প্রবেশপথ ব্যবহার করুন।",
    ...
    ta: {
        ...
        advisoryPlaceholder:
            "எ.கா. முதன்மை கோவில் மண்டலம் முழு கொள்ளளவில் உள்ளது — தயவுசெய்து கிழக்கு நுழைவாயிலைப் பயன்படுத்தவும்.",
    ...
    or: {
        ...
        advisoryPlaceholder:
            "ଉଦା. ମୁଖ୍ୟ ମନ୍ଦିର ମଣ୍ଡଳ ପୂର୍ଣ୍ଣ କ୍ଷମତାରେ ଅଛି — ଦୟାକରି ପୂର୍ବ ପ୍ରବେଶ ଦ୍ୱାର ବ୍ୟବହାର କରନ୍ତୁ।",
...
function loadData() {
    ...
    /*
       ...
       to localStorage before this feature existed.
    */
...
function checkPopularPlacesCrowd() {
    ...
    /*
       These are public sightseeing spots, not the
       on-site AI camera zones, so there's no live
       headcount feed for them. Crowd levels are
       estimated the same way transport and weather
       conditions are simulated elsewhere in this
       prototype.
    */
    ...
    popularPlaces.forEach(place => {
        ...
        if (occupancy < 50) {
            ...
            place.crowdStatus = "WARNING";

            place.crowdLabel =
                `Moderate crowd (${occupancy}%)`;

        }
        else {

            place.crowdStatus = "DANGER";

            place.crowdLabel =
                `High crowd (${occupancy}%)`;
    ...
    showToast(
        "Crowd levels updated for popular places"
    );
...
function useDetectedIncident() {
    ...
    if (zoneSelect) zoneSelect.value = String(zone.id);
...
function updateLiveCrowdIntelligence(count) {
    ...
    if(occupancy>=100){
        badge.textContent="OVER CAPACITY"; badge.style.background="#fee2e2"; badge.style.color="#b91c1c";
        msg.textContent="Crowd count is at or above the configured zone capacity.";
        ...
        badge.textContent=delta>0?"SURGE DETECTED":"RAPID DROP"; badge.style.background="#fef3c7"; badge.style.color="#a16207";
        msg.textContent=delta>0?"A rapid increase in the anonymous camera count was detected.":"A rapid decrease in the anonymous camera count was detected.";
        ...
        badge.textContent="WATCH"; badge.style.background="#fef3c7"; badge.style.color="#a16207";
        msg.textContent="Monitoring shows elevated crowd pressure or unusual variation.";
        statusText.textContent="Elevated activity — keep monitoring.";
    } else {
        badge.textContent="NORMAL"; badge.style.background="#dcfce7"; badge.style.color="#15803d";
        msg.textContent="Crowd pattern is currently within the normal monitoring range.";
        statusText.textContent="No anomaly detected";
    }
